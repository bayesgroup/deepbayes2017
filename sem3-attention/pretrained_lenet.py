...
cnn_feature_layer = lenet['inception_5b/output']
get_cnn_features = theano.function([cnn_input_var], get_output(cnn_feature_layer))

pca = pickle.load(open("./data/svd.pcl"))


def image_to_features(im):
    assert len(im.shape) ==3 and im.shape[2]==3,"You should provide an RGB image of shape [h,w,3]"
    im = preprocess(im)
    cnn_features = get_cnn_features(im)[0]
    H,W = cnn_features.shape[-2:]
    cnn_features_flat = cnn_features.transpose([1,2,0]).reshape([-1,1024])
    svd_features_flat = pca.transform(cnn_features_flat)
    return svd_features_flat.reshape([H,W,-1]).transpose([2,0,1])
